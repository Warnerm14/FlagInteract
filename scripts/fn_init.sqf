...
[] spawn {
	...
	player addAction ["Raise Flag",{_flag = flagTarget; _flag setVariable ["animState",true,true]; if (flagAnimationPhase _flag == 0) then {_flag forceFlagTexture (_flag getVariable ["flagTxt","\A3\Data_F\Flags\Flag_Red_CO.paa"]);}; player playMove "AinvPknlMstpSnonWnonDnon_medic_1"; [_flag, 1] call BIS_fnc_animateFlag; waitUntil {(flagAnimationPhase _flag) == 1}; _flag setVariable ["animState",false,true];},[""],0.1,false,true,"","!(isNull flagTarget) && vehicle player == player && alive player && player getVariable 'canInteractWithFlags' && (flagAnimationPhase flagTarget) < 1 && !(flagTarget getVariable ""animState"") "];
	player addAction ["Raise Flag to Half Mast",{_flag = flagTarget; _flag setVariable ["animState",true,true]; if (flagAnimationPhase _flag == 0) then {_flag forceFlagTexture (_flag getVariable ["flagTxt","\A3\Data_F\Flags\Flag_Red_CO.paa"]);}; player playMove "AinvPknlMstpSnonWnonDnon_medic_1"; [_flag, 0.5] call BIS_fnc_animateFlag; waitUntil {(flagAnimationPhase _flag) == 0.5}; _flag setVariable ["animState",false,true];},[""],0.1,false,true,"","!(isNull flagTarget) && vehicle player == player && alive player && player getVariable 'canInteractWithFlags' && (flagAnimationPhase flagTarget) < 0.5 && !(flagTarget getVariable ""animState"") "];
	player addAction ["Lower Flag to Half Mast",{_flag = flagTarget; _flag setVariable ["animState",true,true]; player playMove "AinvPknlMstpSnonWnonDnon_medic_1"; [_flag, 0.5] call BIS_fnc_animateFlag; waitUntil {(flagAnimationPhase _flag) == 0.5}; _flag setVariable ["animState",false,true];},[""],0.1,false,true,"","!(isNull flagTarget) && vehicle player == player && alive player && player getVariable 'canInteractWithFlags' && (flagAnimationPhase flagTarget) > 0.5 && !(flagTarget getVariable ""animState"")  "];
	player addAction ["Lower Flag",{_flag = flagTarget; _flag setVariable ["animState",true,true]; player playMove "AinvPknlMstpSnonWnonDnon_medic_1"; [_flag, 0] call BIS_fnc_animateFlag; _flag setVariable ["flagTxt",(flagTexture _flag),true]; waitUntil {(flagAnimationPhase _flag) == 0}; _flag forceFlagTexture "\FlagInteract\textures\blank_ico.paa"; _flag setVariable ["animState",false,true];},[""],0.1,false,true,"","!(isNull flagTarget) && vehicle player == player && alive player && player getVariable 'canInteractWithFlags' && (flagAnimationPhase flagTarget) > 0 && !(flagTarget getVariable ""animState"") "];
